...
port_ttyUSB1 = serial.Serial(port='/dev/ttyUSB1', baudrate=115200, timeout=50)
...
for id in selected_rows:
  ...
  update_exitStatus = ('UPDATE InputQueue SET exitStatus = {0}, dateCompleted = \'{1}\' WHERE id = {2}'.format(exit_code, time.strftime('%Y-%m-%d %H:%M:%S'), id))
